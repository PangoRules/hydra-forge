...
// Ensure we have a valid object for the template
const shortcutsByScopeForTemplate = computed(() => {
  return shortcutsByScope.value || {}
})
</script>

<template>
  <UModal v-model:open="show" title="Keyboard Shortcuts">
    <template #body>
      <div class="space-y-6">
        <div
          v-for="(shortcuts, scope) in shortcutsByScopeForTemplate"
          :key="scope"
          class="space-y-3"
        >
          <h3 class="font-medium">{{ scope }} Shortcuts</h3>
