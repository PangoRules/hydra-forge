...
public class EfChatArchiveService(HydraForgeDbContext context) : IChatArchiveService
{
    ...
    private async Task SetProjectArchiveStateAsync(
        Guid projectId,
        DateTime? archivedAt,
        CancellationToken ct
    )
    {
        var chatFolders = await context
            .ChatFolders.Where(f => f.ProjectId == projectId)
            .ToListAsync(ct);
        ...
        var chatSessions = await context
            .ChatSessions.Where(s =>
                s.ProjectId == projectId
                || (s.FolderId.HasValue && folderIds.Contains(s.FolderId.Value))
            )
            .ToListAsync(ct);
