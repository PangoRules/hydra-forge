...
public class EfPlanRepository(HydraForgeDbContext context) : IPlanRepository
{
    ...
    public async Task<IReadOnlyList<Plan>> ListByProjectAsync(
        Guid projectId,
    ...
    public async Task<IReadOnlyList<Plan>> ListByCardAsync(
        Guid cardId,
        PlanListFilter filter,
        CancellationToken ct = default
    )
    ...
    {
        return await _context.PlanVersions.FirstOrDefaultAsync(
    ...
    )
    {
        return await _context
            .PlanVersions.Where(v => v.PlanId == planId)
            .OrderBy(v => v.Version)
            .ToListAsync(ct);
    }
