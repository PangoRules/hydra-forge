...
public static class AttachmentServiceCollectionExtensions
{
    ...
    )
    {
        var provider =
            ...
            ?? "Local";
        var maxBytes = long.TryParse(configuration["FileStorage:MaxBytes"], out var mb)
            ? mb
            : 10_000_000L;
        ...
        {
            var bucketName =
                configuration["FileStorage:S3:BucketName"]
                ?? throw new InvalidOperationException(
                    "FileStorage:S3:BucketName is required when FileStorage:Provider=S3"
                );
            var region =
                configuration["FileStorage:S3:Region"]
                ?? throw new InvalidOperationException("FileStorage:S3:Region is required");
            var accessKey =
                configuration["FileStorage:S3:AccessKey"]
                ?? throw new InvalidOperationException("FileStorage:S3:AccessKey is required");
            var secretKey =
                configuration["FileStorage:S3:SecretKey"]
            ...
            var forcePathStyle =
                bool.TryParse(configuration["FileStorage:S3:ForcePathStyle"], out var fps) && fps;
            ...
            services.AddSingleton<IFileStore>(sp => new S3FileStore(
        ...
        else
        {
            ...
            services.AddSingleton<IFileStore>(sp => new LocalFileStore(
                localPath,
        ...
        services.AddScoped(sp =>
        {
            ...
            var cardRepo = sp.GetRequiredService<Application.Cards.ICardRepository>();
            var memberRepo = sp.GetRequiredService<Application.Projects.IProjectMemberRepository>();
            var auditWriter = sp.GetRequiredService<Application.Audit.IAuditLogWriter>();
            var snapshotRefresher = sp.GetRequiredService<IProjectSnapshotRefresher>();
            var publisher =
                sp.GetRequiredService<Application.Realtime.IProjectBoardEventPublisher>();
