...
public class EfProjectDocumentRepository(HydraForgeDbContext context) : IProjectDocumentRepository
{
    ...
    public async Task<IReadOnlyList<ProjectDocument>> ListByProjectAsync(
        Guid projectId,
        CancellationToken ct = default
    ) =>
        await context
            .ProjectDocuments.Where(d => d.ProjectId == projectId && d.ArchivedAt == null)
            .OrderBy(d => d.DocType)
            .ThenBy(d => d.Title)
            .ToListAsync(ct);

    public async Task<ProjectDocument?> GetByDocTypeAsync(
        Guid projectId,
        ProjectDocType docType,
        CancellationToken ct = default
    ) =>
        await context.ProjectDocuments.FirstOrDefaultAsync(
            d => d.ProjectId == projectId && d.DocType == docType && d.ArchivedAt == null,
            ct
        );
    ...
    public async Task<ProjectDocumentVersion?> GetVersionAsync(
        ...
        await context.ProjectDocumentVersions.FirstOrDefaultAsync(
            v => v.ProjectDocumentId == documentId && v.Version == version,
            ct
        );

    public async Task<IReadOnlyList<ProjectDocumentVersion>> ListVersionsAsync(
        Guid documentId,
        CancellationToken ct = default
    ) =>
        await context
            .ProjectDocumentVersions.Where(v => v.ProjectDocumentId == documentId)
