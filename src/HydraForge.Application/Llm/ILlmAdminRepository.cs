...
{
    Task<List<LlmProvider>> ListProvidersAsync(int skip, int take, string? search, CancellationToken ct = default);
    ...
    Task<ProviderModelConfig?> GetModelConfigAsync(Guid providerId, Guid modelId, CancellationToken ct = default);
    ...
    Task<FeatureRoutingConfig?> GetRoutingByFeatureAsync(AiFeature feature, CancellationToken ct = default);

    Task<(List<TokenUsageRecord> Items, int TotalCount, long TotalInput, long TotalOutput, decimal TotalCost)> QueryTokenUsageAsync(
    ...
    Task<(List<ImageUsageRecord> Items, int TotalCount, int TotalImages, decimal TotalCost)> QueryImageUsageAsync(
        Guid? userId, Guid? projectId, AiFeature? feature, Guid? providerId, string? modelId,
        DateTime? from, DateTime? to, int skip, int take, CancellationToken ct = default);
