...
/// <summary>
/// Generates and persists an AI reply for a user message, broadcasting stream events to
/// whichever SignalR clients happen to be connected via <see cref="IChatBroadcaster"/>.
///
/// Runs as a Hangfire background job (enqueued by <c>ChatMessagesController</c>'s REST
/// trigger and, for parity, <c>ChatHub.SendMessage</c>) rather than inline inside a request
/// or hub method — on a slow/flaky connection (observed: SignalR handshakes taking 90-170s+
/// over some mobile/Tailscale paths), tying generation to the caller's connection lifetime
/// means a dropped connection cancels a reply that would otherwise have completed fine. This
/// way the trigger only needs to survive long enough to enqueue — a fast, tiny DB write —
/// and generation proceeds regardless of what happens to the connection afterward. Clients
/// that are live and joined to the session's group get the real-time stream; clients that
/// aren't (or never connect at all) still get the finished reply on their next REST fetch.
/// </summary>
public sealed class ChatReplyGenerator(
...
)
{
    ...
    {
        var group = broadcaster.Group(sessionId);
        ...
        if (!await HasSessionAccessAsync(session, userId, ct))
        {
            await group.StreamError(
                userMessageId,
        ...
        try
        {
            ...
            try
            {
                await foreach (var chunk in client.StreamChatAsync(request, cts.Token))
                {
                    ...
                    {
                        content += chunk.Delta;
                        await group.StreamDelta(assistantMessageId, chunk.Delta);
                    }
                }
            }
            catch (OperationCanceledException)
            {
                await group.StreamDone(assistantMessageId, null, null, null, null);
                return;
            }
        ...
        {
            logger.LogError(ex, "Chat reply generation failed for session {SessionId}", sessionId);
            await group.StreamError(
                assistantMessageId,
                "INTERNAL_ERROR",
