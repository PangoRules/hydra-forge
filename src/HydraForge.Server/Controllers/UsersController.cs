...
[ApiController]
public class UsersController(IUserRepository userRepo, IProjectMemberRepository memberRepo)
    : ControllerBase
{
    ...
    )
    {
        ...
        if (excludeProjectId.HasValue)
        {
            ...
            excludeIds = [.. members.Select(m => m.UserId)];
        }

        var users = (
            await _userRepo.FindByUsernamesAsync([], string.IsNullOrWhiteSpace(q) ? null : q, limit)
        )
