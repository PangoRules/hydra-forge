...
public class EnumSchemaTransformer : IOpenApiSchemaTransformer
{
    ...
    )
    {
        ...
        if (underlyingType.IsEnum)
        {
            ...
            schema.Format = null;
            schema.Enum = [.. Enum.GetNames(underlyingType).Select(name => (JsonNode)name)];
            ...
            if (
                schema.Default is JsonValue defaultValue
                && defaultValue.TryGetValue<int>(out var defaultInt)
            )
            {
