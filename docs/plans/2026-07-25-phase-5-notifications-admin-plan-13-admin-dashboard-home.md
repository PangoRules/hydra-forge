...
async function loadDashboard() {
  ...
  try {
    // Load users
    const users = await api.GET<any[]>(ApiRoutes.Admin.usersList(0, 1))
    // Load projects
    const projects = await api.GET<any>(ApiRoutes.Admin.projectsList())
    // Load recent audit
    const audit = await api.GET<{ items: any[] }>(`${ApiRoutes.Admin.auditLog()}?take=10`)
    recentAudit.value = audit.items

    stats.value = {
      totalUsers: 0, // Will need count endpoint or pagination total
      activeUsers: 0,
      disabledUsers: 0,
      totalProjects: projects?.totalCount ?? 0,
      activeProjects: 0,
      archivedProjects: 0,
      totalCards: 0,
