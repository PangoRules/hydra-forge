...
public class AdminServiceTests
{
    ...
    public async Task CreateUser_ValidRequest_ReturnsUserDto()
    {
        ...
        var request = new CreateUserRequest("newuser", "Pass123!", "New", "User", "new@test.com", false);
    ...
    public async Task CreateUser_DuplicateUsername_ReturnsFailure()
    {
        ...
        var hasher = new TestPasswordHasher();
        var service = new AdminService(repo, hasher);
        var request = new CreateUserRequest("existing", "Pass123!", "New", "User", "new@test.com", false);
        var result = await service.CreateUserAsync(request);
...
internal class InMemoryAdminUserRepo : IUserRepository
{
    ...
    public Task<User?> FindByUsernameAsync(string username)
        => Task.FromResult(_users.FirstOrDefault(u => u.UsernameNormalized == username.ToLowerInvariant()));
    ...
    public Task<IReadOnlyList<User>> ListAsync(int skip, int take, string? search, CancellationToken ct = default)
    {
        ...
        return Task.FromResult<IReadOnlyList<User>>(query.OrderBy(u => u.Username).Skip(skip).Take(take).ToList());
    }
    public Task<int> CountAsync(string? search, CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(search))
            return Task.FromResult(_users.Count);
        return Task.FromResult(_users.Count(u => u.Username.Contains(search, StringComparison.OrdinalIgnoreCase)));
    }
