...
{
    private static ModelRouter CreateRouter(IRoutingConfigProvider provider) => new(provider, new NullWarnLogger());
    ...
    public async Task ResolveAsync_TierCeiling_CapsToMaxUserTier()
    {
        ...
        provider.AddModel(modelId, providerId, "standard-model", ModelTier.Standard, maxTokens: 4096);
    ...
    public async Task ResolveAsync_TokensExceedModelContextWindow_BumpsToHigherTier()
    {
        ...
        provider.AddModel(Guid.NewGuid(), ecoProviderId, "economy-small", ModelTier.Economy, maxTokens: 512);
        provider.AddModel(Guid.NewGuid(), stdProviderId, "standard-medium", ModelTier.Standard, maxTokens: 4096);
        provider.AddModel(Guid.NewGuid(), premProviderId, "premium-big", ModelTier.Premium, maxTokens: 16384);
    ...
    public async Task ResolveAsync_NoModelFitsAfterAllTiers_ReturnsContextWindowExceeded()
    {
        ...
        provider.AddRouting(AiFeature.DeepResearch, ModelTier.Economy, null);
        provider.AddEnabledProvider(ecoProviderId, "EcoProvider", ModelTier.Economy);
        provider.AddModel(Guid.NewGuid(), ecoProviderId, "eco-small", ModelTier.Economy, maxTokens: 512);
        provider.AddEnabledProvider(stdProviderId, "StdProvider", ModelTier.Standard);
        provider.AddModel(Guid.NewGuid(), stdProviderId, "std-small", ModelTier.Standard, maxTokens: 1024);
        provider.AddEnabledProvider(premProviderId, "PremProvider", ModelTier.Premium);
        provider.AddModel(Guid.NewGuid(), premProviderId, "prem-small", ModelTier.Premium, maxTokens: 2048);
    ...
    public async Task ResolveAsync_FallbackChainSingleHop_ReturnsFallback()
    {
        ...
        provider.AddEnabledProvider(primaryId, "APrimary", ModelTier.Standard, fallbackProviderId: fallbackId);
        provider.AddModel(primaryModelId, primaryId, "primary-model", ModelTier.Standard, maxTokens: 8192);
        provider.AddEnabledProvider(fallbackId, "BFallback", ModelTier.Standard);
        provider.AddModel(fallbackModelId, fallbackId, "fallback-model", ModelTier.Standard, maxTokens: 8192);
    ...
    public async Task ResolveAsync_FallbackChainMultiHop_ReturnsAllFallbacks()
    {
        ...
        var provider = new FakeRoutingConfigProvider();
        provider.AddRouting(AiFeature.DeepResearch, ModelTier.Standard, null);
        provider.AddEnabledProvider(primaryId, "APrimary", ModelTier.Standard, fallbackProviderId: hop1Id);
        provider.AddModel(Guid.NewGuid(), primaryId, "primary-model", ModelTier.Standard, maxTokens: 8192);
        provider.AddEnabledProvider(hop1Id, "BHop1", ModelTier.Standard, fallbackProviderId: hop2Id);
        provider.AddModel(Guid.NewGuid(), hop1Id, "hop1-model", ModelTier.Standard, maxTokens: 8192);
        provider.AddEnabledProvider(hop2Id, "CHop2", ModelTier.Standard);
        provider.AddModel(Guid.NewGuid(), hop2Id, "hop2-model", ModelTier.Standard, maxTokens: 8192);
    ...
    public async Task ResolveAsync_FallbackCycleDetection_StopsAtCycle()
    {
        ...
        var provider = new FakeRoutingConfigProvider();
        provider.AddRouting(AiFeature.DeepResearch, ModelTier.Standard, null);
        provider.AddEnabledProvider(providerA, "AProviderA", ModelTier.Standard, fallbackProviderId: providerB);
        provider.AddModel(Guid.NewGuid(), providerA, "model-a", ModelTier.Standard, maxTokens: 8192);
        provider.AddEnabledProvider(providerB, "BProviderB", ModelTier.Standard, fallbackProviderId: providerA);
        provider.AddModel(Guid.NewGuid(), providerB, "model-b", ModelTier.Standard, maxTokens: 8192);
    ...
    public async Task ResolveAsync_DisabledProviders_AreSkipped()
    {
        ...
        provider.AddModel(Guid.NewGuid(), disabledId, "disabled-model", ModelTier.Standard, maxTokens: 8192);
        provider.AddEnabledProvider(enabledId, "Enabled", ModelTier.Standard);
        provider.AddModel(Guid.NewGuid(), enabledId, "enabled-model", ModelTier.Standard, maxTokens: 8192);
...
{
    private readonly Dictionary<AiFeature, (ModelTier DefaultTier, ModelTier? MaxUserTier)> _routing = new();
    ...
    public void AddEnabledProvider(
        ...
        bool isEnabled = true)
    {
        _providers.Add(new LlmProvider
        {
            Id = id,
            Name = name,
    ...
    public void AddModel(Guid id, Guid providerId, string modelId, ModelTier tier, int? maxTokens = null)
    {
        _models.Add(new ProviderModelConfig
        {
            Id = id,
    ...
    public Task<FeatureRoutingConfig?> GetRoutingConfigAsync(AiFeature feature, CancellationToken ct)
    {
        if (_routing.TryGetValue(feature, out var cfg))
        {
            return Task.FromResult<FeatureRoutingConfig?>(new FeatureRoutingConfig
            {
                Id = Guid.NewGuid(),
                Feature = feature,
    ...
    public Task<IReadOnlyList<(ProviderModelConfig Model, LlmProvider Provider)>> GetEnabledModelsAtTierAsync(
        ModelTier tier, CancellationToken ct)
    {
        var results = _models
            .Where(m => m.IsEnabled && m.Tier == tier)
            .Join(_providers.Where(p => p.IsEnabled),
                m => m.ProviderId,
                p => p.Id,
                (m, p) => (m, p))
            .OrderBy(x => x.p.Name)
        ...
        return Task.FromResult<IReadOnlyList<(ProviderModelConfig Model, LlmProvider Provider)>>(results);
    }
    ...
    public Task<IReadOnlyList<(ProviderModelConfig Model, LlmProvider Provider)>> GetEnabledModelsAtTierForProviderAsync(
        ModelTier tier, Guid providerId, CancellationToken ct)
    {
        var results = _models
            .Where(m => m.IsEnabled && m.Tier == tier && m.ProviderId == providerId)
            .Join(_providers.Where(p => p.IsEnabled && p.Id == providerId),
                m => m.ProviderId,
                p => p.Id,
                (m, p) => (m, p))
            .ToList();
