...
public class PlansControllerTests
{
    ...
    public async Task GetById_NonExistentPlan_Returns404()
    {
        ...
        var request = new HttpRequestMessage(
            HttpMethod.Get,
            $"/api/projects/{projectId}/plans/{Guid.NewGuid()}"
        );
        request.Headers.Add("Authorization", $"Bearer {token}");
...
internal class PlansTestProjectRepository(List<Project> projects) : IProjectRepository
{
...
internal class PlansTestCardRepository(List<Card> cards) : ICardRepository
{
...
internal class PlansTestMemberRepository(List<ProjectMember> members) : IProjectMemberRepository
{
