...
public class LlmAdminControllerTests
{
    ...
    public async Task ListProviders_ReturnsOk()
    {
        ...
        mock.ListProvidersAsync(Arg.Any<int>(), Arg.Any<int>(), Arg.Any<string?>(), Arg.Any<CancellationToken>())
    ...
    public async Task CreateProvider_WithValidInput_ReturnsCreated()
    {
        ...
        mock.CreateProviderAsync(Arg.Any<CreateProviderInput>(), Arg.Any<CancellationToken>())
            .Returns(Result<ProviderDto>.Success(new ProviderDto(
                providerId, "New Provider", "https://api.new.com",
                "OpenAiCompatible", "Text", "Standard", null, true,
        ...
        var input = new CreateProviderInput(
            "New Provider", "https://api.new.com", "sk-test-key",
            "OpenAiCompatible", "Text", "Standard", null);
        var response = await client.PostAsJsonAsync("api/admin/providers", input, CancellationToken.None);
    ...
    public async Task CreateProvider_WithMissingName_ReturnsBadRequest()
    {
        var mock = Substitute.For<ILlmAdminService>();
        mock.CreateProviderAsync(Arg.Any<CreateProviderInput>(), Arg.Any<CancellationToken>())
            .Returns(Result<ProviderDto>.Failure(new Error("PROVIDER_NAME_REQUIRED", "Name required")));
    ...
    public async Task GetProvider_ReturnsOk()
    {
        ...
        mock.GetProviderAsync(Arg.Any<Guid>(), Arg.Any<CancellationToken>())
            .Returns(Result<ProviderDto>.Success(new ProviderDto(
                providerId, "Test Provider", "https://api.test.com",
                "OpenAiCompatible", "Text", "Standard", null, true,
                DateTime.UtcNow, DateTime.UtcNow)));
        ...
        var response = await client.GetAsync($"api/admin/providers/{providerId}", CancellationToken.None);
    ...
    public async Task GetProvider_NotFound_Returns404()
    {
        ...
        var response = await client.GetAsync($"api/admin/providers/{Guid.NewGuid()}", CancellationToken.None);
    ...
    public async Task UpdateProvider_WithValidInput_ReturnsOk()
    {
        var mock = Substitute.For<ILlmAdminService>();
        var providerId = Guid.NewGuid();
        mock.UpdateProviderAsync(Arg.Any<Guid>(), Arg.Any<UpdateProviderInput>(), Arg.Any<CancellationToken>())
            .Returns(Result<ProviderDto>.Success(new ProviderDto(
                providerId, "New Name", "https://api.new.com",
                "OpenAiCompatible", "Text", "Premium", null, true,
                DateTime.UtcNow, DateTime.UtcNow)));
        ...
        var input = new UpdateProviderInput("New Name", "https://api.new.com", null, "Premium", null, null);
        var response = await client.PutAsJsonAsync($"api/admin/providers/{providerId}", input, CancellationToken.None);
    ...
    public async Task UpdateProvider_NotFound_Returns404()
    {
        ...
        var response = await client.PutAsJsonAsync($"api/admin/providers/{Guid.NewGuid()}", input, CancellationToken.None);
    ...
    public async Task DisableProvider_ReturnsNoContent()
    {
        ...
        var response = await client.DeleteAsync($"api/admin/providers/{Guid.NewGuid()}", CancellationToken.None);
    ...
    public async Task CreateModel_WithValidInput_ReturnsCreated()
    {
        ...
        mock.CreateModelAsync(Arg.Any<Guid>(), Arg.Any<CreateModelInput>(), Arg.Any<CancellationToken>())
            .Returns(Result<ProviderModelConfigDto>.Success(new ProviderModelConfigDto(
                modelId, providerId, "gpt-4o", "GPT-4o", "Premium", 0.00001m, 128000, true)));
        ...
        var response = await client.PostAsJsonAsync($"api/admin/providers/{providerId}/models", input, CancellationToken.None);

        Assert.Equal(201, (int)response.StatusCode);
        var dto = await response.Content.ReadFromJsonAsync<ProviderModelConfigDto>(CancellationToken.None);
        Assert.NotNull(dto);
    ...
    public async Task CreateModel_ProviderNotFound_Returns404()
    {
        var mock = Substitute.For<ILlmAdminService>();
        mock.CreateModelAsync(Arg.Any<Guid>(), Arg.Any<CreateModelInput>(), Arg.Any<CancellationToken>())
            .Returns(Result<ProviderModelConfigDto>.Failure(new Error("PROVIDER_NOT_FOUND", "Not found")));
        ...
        var input = new CreateModelInput("gpt-4o", "GPT-4o", "Premium", 0.00001m, 128000, true);
        var response = await client.PostAsJsonAsync($"api/admin/providers/{Guid.NewGuid()}/models", input, CancellationToken.None);
    ...
    public async Task UpdateModel_ReturnsOk()
    {
        ...
        var providerId = Guid.NewGuid();
        var modelId = Guid.NewGuid();
        mock.UpdateModelAsync(Arg.Any<Guid>(), Arg.Any<Guid>(), Arg.Any<UpdateModelInput>(), Arg.Any<CancellationToken>())
            .Returns(Result<ProviderModelConfigDto>.Success(new ProviderModelConfigDto(
                modelId, providerId, "gpt-4o", "GPT-4o Updated", "Premium", 0.00002m, 256000, false)));
        ...
        var response = await client.PutAsJsonAsync($"api/admin/providers/{providerId}/models/{modelId}", input, CancellationToken.None);
    ...
    public async Task DeleteModel_ReturnsNoContent()
    {
        ...
        var response = await client.DeleteAsync($"api/admin/providers/{Guid.NewGuid()}/models/{Guid.NewGuid()}", CancellationToken.None);
    ...
    public async Task ListRouting_ReturnsOk()
    {
        ...
        mock.ListRoutingAsync(Arg.Any<CancellationToken>())
            .Returns(Result<IReadOnlyList<FeatureRoutingDto>>.Success(
                new List<FeatureRoutingDto> { new(Guid.NewGuid(), AiFeature.PersonalChat, "Standard", null, DateTime.UtcNow, DateTime.UtcNow) }));
        ...
        Assert.Equal(200, (int)response.StatusCode);
        var dtos = await response.Content.ReadFromJsonAsync<IReadOnlyList<FeatureRoutingDto>>(JsonOptions, CancellationToken.None);
        Assert.NotNull(dtos);
    ...
    public async Task UpdateRouting_ReturnsOk()
    {
        ...
        mock.UpdateRoutingAsync(Arg.Any<string>(), Arg.Any<UpdateRoutingInput>(), Arg.Any<CancellationToken>())
            .Returns(Result<FeatureRoutingDto>.Success(new FeatureRoutingDto(
                featureId, AiFeature.PersonalChat, "Premium", "Economy", DateTime.UtcNow, DateTime.UtcNow)));
        ...
        var response = await client.PutAsJsonAsync($"api/admin/routing/PersonalChat", input, CancellationToken.None);

        Assert.Equal(200, (int)response.StatusCode);
        var dto = await response.Content.ReadFromJsonAsync<FeatureRoutingDto>(JsonOptions, CancellationToken.None);
        Assert.NotNull(dto);
    ...
    public async Task GetBudget_ReturnsOk()
    {
        ...
        mock.GetBudgetAsync(Arg.Any<Guid>(), Arg.Any<CancellationToken>())
            .Returns(Result<UserBudgetDto>.Success(new UserBudgetDto(
                userId, null, 1000, 100000, 500, 50000, 100, DateTime.UtcNow, DateTime.UtcNow.AddMonths(1))));

        using var factory = CreateFactory(mock);
        var client = factory.CreateClient();
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", AdminToken);

        var response = await client.GetAsync($"api/admin/users/{userId}/budget", CancellationToken.None);
    ...
    public async Task UpdateBudget_ReturnsOk()
    {
        var mock = Substitute.For<ILlmAdminService>();
        var userId = Guid.NewGuid();
        mock.UpdateBudgetAsync(Arg.Any<Guid>(), Arg.Any<UpdateBudgetInput>(), Arg.Any<CancellationToken>())
            .Returns(Result<UserBudgetDto>.Success(new UserBudgetDto(
                userId, null, null, 200000, 500, 500, 0, DateTime.UtcNow, DateTime.UtcNow.AddMonths(1))));
        ...
        var response = await client.PutAsJsonAsync($"api/admin/users/{userId}/budget", input, CancellationToken.None);
    ...
    public async Task QueryTokenUsage_ReturnsOk()
    {
        ...
        mock.QueryTokenUsageAsync(
                ...
                Arg.Any<DateTime?>(), Arg.Any<int>(), Arg.Any<int>(), Arg.Any<CancellationToken>())
        ...
        Assert.Equal(200, (int)response.StatusCode);
        var dto = await response.Content.ReadFromJsonAsync<TokenUsagePageDto>(CancellationToken.None);
        Assert.NotNull(dto);
    }
    ...
    public async Task QueryImageUsage_ReturnsOk()
    {
        ...
        Assert.Equal(200, (int)response.StatusCode);
        var dto = await response.Content.ReadFromJsonAsync<ImageUsagePageDto>(CancellationToken.None);
        Assert.NotNull(dto);
    }
    ...
    public async Task GetModel_ReturnsOk()
    {
        ...
        var response = await client.GetAsync($"api/admin/providers/{providerId}/models/{modelId}", CancellationToken.None);
    ...
    public async Task GetModel_NotFound_Returns404()
    {
        var mock = Substitute.For<ILlmAdminService>();
        mock.GetModelAsync(Arg.Any<Guid>(), Arg.Any<Guid>(), Arg.Any<CancellationToken>())
            .Returns(Result<ProviderModelConfigDto>.Failure(new Error("MODEL_NOT_FOUND", "Not found")));

        using var factory = CreateFactory(mock);
        var client = factory.CreateClient();
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", AdminToken);

        var response = await client.GetAsync($"api/admin/providers/{Guid.NewGuid()}/models/{Guid.NewGuid()}", CancellationToken.None);
...
internal class LlmAdminTestWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        {
            var descriptor = services.SingleOrDefault(d => d.ServiceType == typeof(ILlmAdminService));
