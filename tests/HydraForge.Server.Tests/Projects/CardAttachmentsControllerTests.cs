...
public class CardAttachmentsControllerTests
{
    ...
    public async Task Upload_NonMember_Returns403()
    {
        ...
        using var client = factory.CreateClient();
        var userId = Guid.NewGuid();
        var token = AttachmentsTestWebApplicationFactory.IssueToken(userId, "user", isAdmin: false);
        var projectId = Guid.NewGuid();
        var cardId = Guid.NewGuid();
    ...
    public async Task Download_NotFound_Returns404()
    {
        ...
        var request = new HttpRequestMessage(
            HttpMethod.Get,
            $"/api/projects/{projectId}/cards/{cardId}/attachments/{Guid.NewGuid()}"
        );
        request.Headers.Add("Authorization", $"Bearer {token}");
...
internal class AttachmentsTestWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureServices(services =>
        {
            ...
            services.AddScoped<ICardWatcherRepository>(
                _ => new AttachmentsTestCardWatcherRepository()
            );
            services.AddScoped<ICardRelationshipRepository>(
                _ => new AttachmentsTestCardRelationshipRepository()
            );
            services.AddScoped<IProjectMemberRepository>(
                _ => new AttachmentsTestProjectMemberRepository(_members)
            );
            services.AddScoped<Application.Auth.IUserRepository>(
                _ => new AttachmentsTestUserRepository(_users)
            );
            services.AddScoped<IProjectContextSnapshotRepository>(
                _ => new AttachmentsTestSnapshotRepository()
            );
            ...
            services.AddScoped(sp =>
            {
                ...
                var snapshotRefresher =
                    sp.GetRequiredService<Application.ProjectSnapshots.IProjectSnapshotRefresher>();
...
internal class AttachmentsTestProjectRepository(List<Project> projects) : IProjectRepository
{
...
internal class AttachmentsTestColumnRepository(List<Column> columns) : IColumnRepository
{
...
internal class AttachmentsTestCardRepository(List<Card> cards) : ICardRepository
{
...
internal class AttachmentsTestUserRepository(List<User> users) : Application.Auth.IUserRepository
{
...
internal class AttachmentsTestSnapshotRefresher
    : Application.ProjectSnapshots.IProjectSnapshotRefresher
{
