...
## Current Phase — Phase 4: Project Space — TUI (in progress)
...
Phase 4 (TUI) is **underway** — see `docs/specs/phase-4-tui.md` for the full task list and live status. Tasks 1-9 done: Spectre.Console scaffolding + NSwag codegen, `ConfigStore` (JWT at `.hydraforge/config.json` in the repo root, 0600 — see D-49), `ApiClientFactory` (NSwag client + token refresh), login screen + startup auth flow, connection handling (`LockScreen` + `ConnectionManager`, auto-retry backoff `[5s,10s,30s,60s]`), project list view, board view (column/card layout + keyboard nav), SignalR integration (BoardHub + PresenceHub), card detail view (sections, `$EDITOR`, checklist, comments). Task 10 (create/edit/move cards via keyboard) is next — plan at `docs/plans/2026-07-24-phase-4-tui-plan-10-card-crud.md`. Tasks 11-17 (dependency panel, blocked indicator, spec/plan viewer, comments, checklists, keyboard reference, status bar) not started — plans already exist in `docs/plans/2026-07-24-phase-4-tui-plan-*.md`, written ahead of execution. Plan files are deleted once their task ships (see git history — `docs/plans/` only holds plans for undone tasks; done tasks' history lives in the PR, not a lingering plan doc). Infra pre-phase decisions (API client strategy, SignalR client wiring, JWT config storage) are resolved — see D-48 in `docs/DECISIONS.md` (credential storage location amended by D-49: repo-relative `.hydraforge/`, not the home directory). `docs/functional-spec.md` §25 Phase 4 has the top-level checklist (kept in sync with `docs/specs/phase-4-tui.md`'s task-level detail).
