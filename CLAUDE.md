...
## Current Phase — Phase 6 in progress (Task 16: LLM Admin Service complete)
...
Phase 6 (LLM Infrastructure) is **in progress**. Pre-phase blocking decisions resolved: nightly job scheduler is **Hangfire + `Hangfire.PostgreSql`** (D-57), `AiNarrative` display surface is Web UI modal + TUI viewer (D-58). Task 2 **complete** (2026-07-30): `IKeyVault` port in Application, `AesGcmKeyVault` impl in Infrastructure (AES-256-GCM, `Llm:EncryptionKey` config, startup validation), migration `20260731000000_ReencryptLlmProviderApiKeys` re-encrypts placeholder rows idempotently, `LlmServiceCollectionExtensions.AddLlmInfrastructure` registered via `PersistenceServiceCollectionExtensions`. See D-59 in `docs/DECISIONS.md`. Task 3 **complete** (2026-07-30): All Application LLM ports defined — `ILlmClient`, `IImageClient`, `IEmbeddingClient`, `IModelRouter`, `IContextCompressor`, `IUsageRecorder`, `ILlmAdminService`, `ILlmClientFactory` + DTOs in `HydraForge.Application/Llm/`. Task 4 **complete** (2026-07-30): `OpenAiCompatibleAdapter` (SSE stream parse, cache-block prefix hashing, `UsageSnapshot` from final chunk, `GetModelsAsync`, `SupportsToolCalling`). Task 5 **complete** (2026-07-30): `AnthropicAdapter` (`cache_control` ephemeral blocks, `/v1/messages` stream, `message_delta` usage mapping, `input_schema` JSON-Schema tool shape, 4-cache-breakpoint limit). Task 7 **complete** (2026-08-01): `DallEAdapter` (`/images/generations` + `/images/edits` inpaint, `InpaintRequest` DTO refined to `ImageBytes`/`MaskBytes`/`Count`). Task 9 **complete** (2026-08-01): `ComfyUiAdapter` (workflow API `/prompt` + poll `/history`, text-to-image + inpaint workflows, 5-min timeout, 2s polling, serves both `ComfyUi` + `Diffusers` adapter types). Task 13 **complete** (2026-08-01): `EfUsageRecorder` (EF-backed `IUsageRecorder`, writes `TokenUsageRecord`/`ImageUsageRecord`, accrues `UserTokenBudget` counters, lazy period rollover) + unique index migration + 7 Npgsql tests (D-61 for the unique-index-over-transaction decision). Task 16 **complete** (2026-08-01): `ILlmAdminService` + `ILlmAdminRepository` (Application port) + `EfLlmAdminRepository` (Infrastructure) + `LlmAdminController` (admin endpoints behind `AdminRequired`) + `LlmAdminControllerTests` (21 tests, NSubstitute mock of `ILlmAdminService`). Plan said to depend on `HydraForgeDbContext` directly but actual impl uses `ILlmAdminRepository` per Clean Architecture rules — new `DomainErrorCodes.Validation` static class added for validation error codes. Task 21 **complete** (2026-08-02): `Hangfire.AspNetCore` + `Hangfire.PostgreSql` packages, `Program.cs` wired (gated on `!Test` env), `/hangfire` dashboard behind `AdminRequiredAuthFilter` (cookie auth reads `auth_token` cookie for `/hangfire` paths), `SystemSettings.AiNarrativeGenerationTimeUtc` field + migration `20260802053522_AddAiNarrativeGenerationTime` + `SetAiNarrativeGenerationTime` method, admin settings endpoint updated, Web UI time picker for AI Narrative Generation Time (UTC). See Hangfire patterns below for wiring details.
