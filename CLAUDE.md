...
Phase 6 (LLM Infrastructure) has not started. Per `docs/functional-spec.md` §25, it has a pre-phase decision blocking it: pick a nightly job scheduler (`BackgroundService` vs. Hangfire vs. Quartz.NET — recommendation is `BackgroundService` for MVP) before implementing `ProjectContextSnapshot.AiNarrative` or any other scheduled work.
