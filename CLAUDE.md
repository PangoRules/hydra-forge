...
Phase 4 (TUI) is **underway** — see `docs/specs/phase-4-tui.md` for the full task list and live status. Tasks 1-4 done: Spectre.Console scaffolding + NSwag codegen, `ConfigStore` (JWT at `.hydraforge/config.json` in the repo root, 0600 — see D-49), `ApiClientFactory` (NSwag client + token refresh), login screen + startup auth flow. Task 5 (connection handling) partially done — `LockScreen` + `ConnectionManager` gate startup on server reachability with auto-retry backoff `[5s,10s,30s,60s]`; the status-bar connection indicator sub-item is deferred to Task 17 (no status bar exists yet) and mid-session disconnect detection needs the Task 7 board loop to hook into, so Task 5 stays unchecked in `docs/specs/phase-4-tui.md` until those land. Task 6 onward (project list, board view, SignalR, card detail/CRUD, dependency panel, spec/plan viewer, etc.) not started — implementation plans for all of them already exist in `docs/plans/2026-07-24-phase-4-tui-plan-*.md`, written ahead of execution. Infra pre-phase decisions (API client strategy, SignalR client wiring, JWT config storage) are resolved — see D-48 in `docs/DECISIONS.md` (credential storage location amended by D-49: repo-relative `.hydraforge/`, not the home directory). `docs/functional-spec.md` §25 Phase 4 has the top-level checklist (kept in sync with `docs/specs/phase-4-tui.md`'s task-level detail).
