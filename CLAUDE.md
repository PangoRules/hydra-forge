...
- **`GET /api/chat/sessions` default scope is `mine`** — when broadening from "owner-only" to "participated-in", unscoped callers (personal `/chats` page, FAB dock History panel) must keep `scope=mine` default. Only project-specific views (`ProjectChatsTab.vue`) opt into `scope=participated`. The fix for this regression required adding `scope` + `types` query params and updating all call sites — caught in testing after Plan 20 shipped.
...
- **Vue watcher emit guard — prevent re-entrant loops** — When a child component syncs a prop to a local ref and watches the ref to emit changes back to the parent, a loop can form: parent PATCHes → updates prop → watcher syncs ref → watcher emits → parent PATCHes again. Guard with `if (oldValue !== v)` in the watcher — when the parent syncs back the same value, `oldValue === v` breaks the loop. Example in `ChatSessionHeader.vue`'s `selectedPersonalityId` watcher. Also guard mount-time emit: `oldValue !== undefined && oldValue !== v` if the initial emit would cause a spurious PATCH.
