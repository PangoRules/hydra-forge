...
- Phase 3 (Project Space — Web UI) is in progress on `feat/phase-3-web-ui`. Plan 1 (auth + scaffold), Plan 2 (project list + board view), and Plan 3 (card modal core) are complete on `task/phase-3-card-modal-core`: CardModal with desktop two-column + mobile tabbed layout, CardDescription with Tiptap markdown editor + debounced auto-save, CardMetadata sidebar, archive/restore card from modal, reusable `AppModal` wrapper. Also includes board filtering/UX polish, session expiry warning system with `POST /api/Auth/refresh` backend endpoint, auth hardening (sync `checkAuth`, `ClientOnly` hydration fix, cookie deletion hardening), and archived card read-only mode. 67 vitest component/store/composable/page tests, 446 xUnit tests.
- Plans 4-6 (card modal panels, specs/plans, realtime SignalR, polish/hardening) remain. Two additional plans exist but not yet executed: `docs/superpowers/plans/2026-06-25-phase-3-card-modal-hardening.md` (useApi try/catch, version ownership, shared utilities, a11y) and `docs/superpowers/plans/2026-06-25-phase-3-e2e-testing-foundation.md` (Playwright). Plan 4's Task 16 depends on the hardening plan's Task 2/3.
- Project-management gaps remain within Phase 3 scope (no timeline): project edit UI, project creation with initial members, project list beautification. Consider a Plan 7 or integrate into Plan 6 (polish/hardening).
