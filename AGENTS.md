...
- **closeTopmost dock-kind guard** — `usePopupZIndex.closeTopmost()` must NOT unregister `kind: 'dock'` entries from the stack. Dock entries register once in `onMounted` and never re-register — dropping them from the stack would strand them permanently. Use `if (top.kind === 'dock') return` after calling `top.close()` to keep them reachable for the next Escape press.
- **localStorage persist watcher flush mode** — Default `'post'` flush is correct for Pinia store watchers that persist state to localStorage. Add `await nextTick()` in tests before simulating a hard reload (destroying/recreating the store) to let the batched flush complete. `flush: 'sync'` can cause test-timing issues (watcher fires during setup before store init finishes) and offers no real durability edge — a tab kill before the flush is equally unrecoverable with sync.
