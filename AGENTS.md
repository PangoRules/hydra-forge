...
- Phase 3 (Project Space — Web UI) is in progress on `feat/phase-3-web-ui`. Plan 1 (auth + scaffold) and Plan 2 (project list + board view) are complete. Plan 2 delivers: Pinia board store with optimistic card moves + rollback, project list page (`/projects`), project create modal with git remote URL + provider fields (advanced expander), board view (desktop kanban + mobile list with `md:` breakpoint switching), project name in board header. `vue-draggable-plus` was removed due to SSR incompatibility with Nuxt 4 — plain `v-for` used instead. Card creation UI not built yet (curl-only for now). Plans 3-6 (card modal, specs/plans, realtime SignalR, polish/hardening) remain.
...
- 43 vitest tests across 11 test files for web UI (stores, composables, components, middleware, pages).
...
- **Nuxt UI v4 UModal**: Use `v-model:open` for two-way binding (not `:open`). Content goes in named slots (`#body`, `#header`, `#footer`) — the default slot is a `DialogTrigger`, not modal content. No `UOverlay` component exists in v4; the overlay is built into `UModal` via the `overlay` prop (defaults to `true`).
...
- **Toast system** — Nuxt UI v4 `useToast()` composable. `Toaster` component must be placed once in `layouts/default.vue` inside `<UApp>`. Must be explicitly imported: `import Toaster from '@nuxt/ui'` (not auto-imported, not a named export). Never use `console.log`/`console.error`/`console.warn` in production code — all user-facing feedback uses toasts.
