...
- Two additional plans were added 2026-06-25, not yet executed: `docs/superpowers/plans/2026-06-25-phase-3-card-modal-hardening.md` (fixes the `useApi()` try/catch bug across 3 components, lifts card-version ownership to `CardModal`, extracts `lib/card-type.ts`/`lib/date.ts`) and `docs/superpowers/plans/2026-06-25-phase-3-e2e-testing-foundation.md` (Playwright). Plan 4's Task 16 (`docs/superpowers/plans/2026-06-23-phase-3-plan-4-card-modal-panels.md`) was rewritten in the same pass to depend on the hardening plan's Task 2/3 — read that plan's pre-execution note before starting any of its tasks.
