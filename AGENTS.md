...
- Phase 6 (LLM Infrastructure) in progress on `feat/phase-6-llm-infrastructure`. Tasks 1-12 complete: `IKeyVault` + `AesGcmKeyVault` + startup validation, Application ports (`ILlmClient`/`IImageClient`/`IEmbeddingClient`/`IModelRouter`/`IContextCompressor`/`IUsageRecorder`/`ILlmAdminService`/`ILlmClientFactory`) + DTOs, 5 adapters (OpenAiCompatible/Anthropic/Ollama/DallE/StabilityAi/ComfyUi), `LlmClientFactory`, `ModelRouter`, `ContextCompressor`, `FeatureRoutingConfig` startup seeder. Tasks 13-14, 16-24 pending: usage recording, budget enforcement, admin UI, Hangfire, AiNarrative job, docs.
