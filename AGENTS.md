...
- **Toast system** — Nuxt UI v4 `useToast()` composable. `Toaster` component must be placed once in `layouts/default.vue` inside `<UApp>`. Never use `console.log`/`console.error`/`console.warn` in production code — all user-facing feedback uses toasts.
